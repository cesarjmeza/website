...
var arxiv_authorid = "0000-0003-4085-1849";
...
function makearXiv(feed) {
  ...
  var html =
    '<div id="arxivcontainer" style="font-family:Lucida Grande,helvetica, arial, verdana,sans-serif;margin:.7em;font-size:90%">\n';
  var format_name = "";
  ...
  if (arxiv_includeTitle !== 0) {
    ...
    html +=
      '<h3 style="padding-bottom:2px; margin-bottom:3px;">' +
      htmlFix(feed.title) +
      "</h3>\n";
  }
  ...
  for (x = 0; x < num_entries; x++) {
    ...
    html +=
      '\t<span class="list-identifier" style="font-weight:bold"><a href="' +
      feed.entries[x].id +
      '" title="Abstract">' +
      feed.entries[x].id +
      "</a> [ ";
    ...
    html +=
      '\t\t<div class="list-authors" style="font-weight:normal;font-size:110%;text-decoration:none;">' +
      feed.entries[x].authors +
      "</div>\n";
    //Add coments in a div only if available
    if (
      arxiv_includeComments &&
      feed.entries[x].comment &&
      feed.entries[x].comment.length > 1
    ) {
      html +=
        '\t\t<div class="list-comments" style="font-weight:normal;font-size:90%;"><span class="descriptor">Comments:</span> ' +
        feed.entries[x].comment +
        "</div>\n";
    }
    ...
    if (arxiv_includeSubjects) {
      html +=
        '\t\t<div class="list-subjects" style="font-size:90%;"><span class="descriptor">Subjects:</span> <span class="primary-subject" style="font-weight:bold;">' +
        feed.entries[x].subject +
        "</span>";
    ...
    if (
      ...
      feed.entries[x].journal_ref &&
      feed.entries[x].journal_ref.length > 1
    ) {
      html +=
        '\t\t<div class="list-journal-ref" style="font-weight:normal;font-size:90%;"><span class="descriptor">Journal ref:</span> ' +
        feed.entries[x].journal_ref +
        "</div>\n";
    }
    ...
    if (
      arxiv_includeDOI &&
      feed.entries[x].doi &&
      feed.entries[x].doi.length > 0
    ) {
      html +=
        '\t\t<div class="list-doi" style="font-weight:normal;font-size:90%;"><span class="descriptor">DOI:</span> ';
      var dois = feed.entries[x].doi.split(" ");
      for (var j in dois) {
        html +=
          '<a href="https://dx.doi.org/' + dois[j] + '">' + dois[j] + "</a> ";
      }
      html += "</div>\n";
    }
  ...
  if (extra_entries) {
    html +=
      '<br /><span style="font-size:80%">[ Showing ' +
      num_entries +
      " of " +
      feed.entries.length +
      ' total entries, additional <a href="https://arxiv.org/a/' +
      arxiv_authorid +
      '">' +
      (feed.entries.length - num_entries) +
      "</a> entries available at arXiv.org ]</span>";
  } else {
    html +=
      '<br /><span style="font-size:80%">[ Showing ' +
      num_entries +
  ...
  html +=
    '<br /><span id="authorid_hook" style="font-size:80%; padding-left:0px">[ This list is powered by an <a href="https://arxiv.org/a/' +
    arxiv_authorid +
    '">arXiv author id</a> and the <a href="https://arxiv.org/help/myarticles">myarticles</a> widget ]</span>';
...
function makePrettyarXiv(feed) {
  ...
  var html =
    '<div id="arxivcontainer" style="width:250px; border:solid 1px #' +
    arxiv_border_color +
    ";font-family:sans-serif;font-size:10px; background-color:#" +
    arxiv_bg_color +
    ';">';
  if (arxiv_includeTitle == 1) {
    html +=
      '<div id="feedtitle" style="font-size:14px; padding-top:5px; padding-bottom:5px; text-align:center;">';
    //Title has an &apos; in it which won't display correctly on some browsers (IE being
    //the prime culprit this time), use htmlFix to fix that.
  ...
  for (x = 0; x < num_entries; x++) {
    ...
    html +=
      '<div class="entry" style="padding-left:6px;width:244px;background-color:#' +
      arxiv_entry_color +
      '">';
    //Title and authors
    html +=
      ...
      "</a></div>";
    html +=
      '<div class="entryauthors" style="margin-left:0.6em">' +
      feed.entries[x].authors +
      "</div>";
    //add summary only if desired
    if (arxiv_includeSummary == 1) {
      html +=
        '<div class="entrysummary" style="margin-left:0.6em">' +
        feed.entries[x].summary +
        "</div>";
    }
    //end the entry div
    html += "</div>";
  ...
  html +=
    '<a href="https://arxiv.org/help/myarticles">myarticles</a> is powered by <a href="https://arxiv.org/a/' +
    arxiv_authorid +
    '">arXiv author id</a> data';
  html += "</div>";
